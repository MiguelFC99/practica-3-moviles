...
class ApplicationBloc extends Bloc<ApplicationEvent, ApplicationState> {
  ...
  Future<void> _imgLabeling(File imageFile) async {
    ...
    //codigicar imagen a base 64
    List<int> imageByt = imageFile.readAsBytesSync();
    String b64Image = base64Encode(imageByt);

    for (var item in labels) {
      ImageLabelItem valTemp = ImageLabelItem(
          imagenBase64: b64Image,
          similitud: item.confidence,
          identificador: item.entityId,
          texto: item.text);
      _listLabeledItems.add(valTemp);
    }
  }
  ...
  Future<void> _barcodeScan(File imageFile) async {
    ...
    for (var item in codes) {
      BarcodeItem valTemp = BarcodeItem(
          imagenBase64: b64Image,
